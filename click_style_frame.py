...
class ClickStyleFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.mouse_button_dropdown = customtkinter.CTkOptionMenu(self, values=[MOUSE_1_CHOICE, MOUSE_2_CHOICE, MOUSE_3_CHOICE], command=self.mouse_button_callback)
        self.mouse_button_dropdown.grid(row=0, column=1, padx=ITEM_PADDING, pady=ITEM_PADDING, sticky="ew")
        self.master.setvar("MOUSE_BUTTON", MOUSE_1_CHOICE)
        ...
        self.clicks_per_event_entry = IntegerEntry(self, variable_name="CLICKS_PER_EVENT", default_value=DEFAULT_CLICKS_PER_EVENT, max_length=MAX_CLICKS_PER_EVENT_DIGITS)
        ...
        self.click_events_entry = IntegerEntry(self, variable_name="CLICK_EVENTS", default_value=DEFAULT_CLICK_EVENTS, min_value=DEFAULT_CLICK_EVENTS, max_length=MAX_CLICK_EVENTS_DIGITS)
        self.click_events_entry.grid(row=2, column=1, padx=ITEM_PADDING, pady=ITEM_PADDING, sticky="ew")

    def mouse_button_callback(self, option_value) -> None:
        self.master.setvar("MOUSE_BUTTON", option_value)
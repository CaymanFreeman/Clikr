...
class ClickLengthFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master: customtkinter.CTk, language_handler: LanguageHandler):
        ...
        self.length_label = VariableLabel(
            self, language_handler=language_handler, label_key="CLICK_LENGTH_LABEL"
        )
        self.length_label.grid(
            row=0, column=0, padx=item_padding, pady=item_padding, sticky="ew"
        )

        self.length_entry = IntegerEntry(
            self,
            variable_name="CLICK_LENGTH",
        ...
        self.length_timescale_dropdown = VariableDropdown(
            self,
        ...
        self.length_timescale_dropdown.configure(
            variable=tkinter.StringVar(
                value=language_handler.scales[master.getvar(name="CLICK_LENGTH_SCALE")]
            )
        )
        self.length_timescale_dropdown.grid(
            row=0, column=2, padx=item_padding, pady=item_padding, sticky="ew"
        )

    def length_timescale_callback(self, option_value) -> None:
        self.master.setvar(
            name="CLICK_LENGTH_SCALE", value=self.language_handler.scales[option_value]
        )

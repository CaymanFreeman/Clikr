...
class ClickIntervalFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master: customtkinter.CTk, language_handler: LanguageHandler):
        ...
        self.interval_label = VariableLabel(
            self, language_handler=language_handler, label_key="CLICK_INTERVAL_LABEL"
        )
        self.interval_label.grid(
            row=0, column=0, padx=item_padding, pady=item_padding, sticky="ew"
        )

        self.interval_entry = IntegerEntry(
            self,
            variable_name="CLICK_INTERVAL",
        ...
        self.interval_timescale_dropdown = VariableDropdown(
            self,
        ...
        self.interval_timescale_dropdown.configure(
            variable=tkinter.StringVar(
                value=language_handler.scales[
                    master.getvar(name="CLICK_INTERVAL_SCALE")
                ]
            )
        )
        self.interval_timescale_dropdown.grid(
    ...
    def interval_timescale_callback(self, option_value: str) -> None:
        self.master.setvar(
            ...
            value=self.language_handler.scales[option_value],
        )

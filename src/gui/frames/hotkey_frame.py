...
class HotkeyFrame(customtkinter.CTkFrame):
    def __init__(
        ...
        self.hotkey_label = VariableLabel(
            self, language_handler=language_handler, label_key="HOTKEY_LABEL"
        )
        self.hotkey_label.grid(
        ...
        keyboard.add_hotkey(
            self.hotkey_value, lambda: self.start_stop_frame.hotkey_toggle()
        )
        self.hotkey_textbox.grid(
            row=0, column=1, padx=item_padding, pady=item_padding, sticky="ew"
        )

        self.change_hotkey_button = VariableButton(
            self,
            language_handler=language_handler,
            label_key="CHANGE_HOTKEY_LABEL",
            command=self.change_hotkey_callback,
        )
        self.change_hotkey_button.grid(

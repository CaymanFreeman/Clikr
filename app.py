...
from label_constants import TITLE
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.location_frame = LocationFrame(master=self, appearance_variables=appearance_variables)
        self.location_frame.grid(row=4, column=0, padx=frame_padding, pady=frame_padding, sticky="ew")

        self.start_stop_frame = StartStopFrame(master=self, appearance_variables=appearance_variables)
        self.start_stop_frame.grid(row=6, column=0, padx=frame_padding, pady=frame_padding, sticky="ew")

        self.hotkey_frame = HotkeyFrame(master=self, start_stop_frame=self.start_stop_frame, appearance_variables=appearance_variables)
